    ...
    public function aprobarSalidaNormal(Request $req){         
            ...
            if ($validator->fails()) {
                ...
                if(Salida::find($id)!=null){
                    ...
                    $ing =  Salida::find($id);        
                    $validator = Validator::make($req->all(), [
                        'cedulaNombreOficiaSalida' => 'required|numeric', 
                        'nombreOficiaSalida'       => 'required|string'
                    ]);
                    ...
                    $ing->cedulaNombreOficiaSalida = $req->input("cedulaNombreOficiaSalida");
                    $ing->nombreOficiaSalida = $req->input("nombreOficiaSalida");
                    if ($validator->fails()) {
                        $data =["estado"=>"error","mensaje"=>"Cédula y nombre de usuario aprueba es obligatorio"];            
                        return response($data,404);                  
                    }else{
        ...
        if ($validator->fails()) {
            ...
            return response($data,404);
               ...
               if($user["datoSalida"]=="entrega"){
                    ...
                    ->update(array('id_salida' => null,'Estado' => "Bodega"));
                    $user->delete();
                    $data =["estado"=>"ok","mensaje"=>"Salida eliminado exitosamente"];            
                    return response($data,200);
    ...
    public function restoreSalida(Request $req){
        ...
        if ($validator->fails()) {
            ...
            if($user && $user->deleted_at !=null){//verifica que el usuario cumpla las condciones         
                ...
                $res->save();
                $data =["estado"=>"ok","mensaje"=>"Salida restaurado con exito"]; 
                return response($data,200);
